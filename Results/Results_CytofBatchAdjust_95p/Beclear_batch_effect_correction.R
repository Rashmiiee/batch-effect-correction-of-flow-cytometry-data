setwd("~/Desktop/Flow_Cytometry_Trail/Scripts_and_Data_for_MDS and markerintensity_plots/T cell panel/STIMULATED_RA_HC_/Input data")
library(readxl)
library(flowCore)
...
head(expr_median_sample_tbl)
expr_median_sample <- t(expr_median_sample_tbl[, -1])
colnames(expr_median_sample) <- expr_median_sample_tbl$sample_id
head(expr_median_sample)
library(readxl)
sample_annotation <- read_excel("sample_annotation.xlsx")
View(sample_annotation)  
library(BEclear)
samples <- as.data.frame(sample_annotation)
df2 <- as.data.frame(expr_median_sample)
batchEffect <- calcBatchEffects(
data = expr_median_sample, samples = samples,
...
summary <- calcSummary(medians = mdifs, pvalues = pvals)
knitr::kable(summary, caption = 'Summary over the batch affected marker-sample combination of the example data set')
score <- calcScore(expr_median_sample,samples, summary, dir = getwd())
knitr::kable(score, caption = 'Batch scores of the example data-set')
cleared.data <- clearBEgenes(expr_median_sample,samples, summary)
corrected.data <- imputeMissingData(cleared.data,rowBlockSize = 60,colBlockSize = 60, epochs = 50,outputFormat = "", dir = getwd())
corrected.data.valid<-replaceOutsideValues(corrected.data)
head(corrected.data.valid)
head(corrected.data)
library(ggrepel)
...
ggplot(ggdf, aes(x = MDS1, y = MDS2, color = condition) +
  ...
  theme_bw() +
  scale_color_manual(values = color_conditions))mds <- plotMDS(corrected.data, plot = FALSE)

ggdf <- data.frame(MDS1 = mds$x, MDS2 = mds$y, sample_id = colnames(corrected.data))
mm <- match(ggdf$sample_id, md$sample_id)
ggdf$condition <- md$'Condition(RA/HC)'[mm]
...
makeBoxplot(expr_median_sample,samples, score,
            bySamples = TRUE,
            col = "standard", main = "T-cell stimulated data", xlab = "Batch",
            ylab = "Beta value", scoreCol = TRUE)
makeBoxplot(corrected.data,samples, score,
            bySamples = TRUE,
            col = "standard", main = "Corrected T-cell stimulated data",
