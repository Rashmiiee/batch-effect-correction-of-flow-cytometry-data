setwd("~/Desktop/Flow_Cytometry_Trail/Scripts_and_Data_for_MDS and markerintensity_plots/T cell panel/STIMULATED_Responders_and_Non_Responders/Input_data")
library(readxl)
library(flowCore)
md <- read_excel("metadata_file.xlsx")
md$respond_level <- factor(md$respond_level,levels = c("responder","non"))
color_conditions <- c("#6A3D9A", "#FF7F00")
names(color_conditions) <- levels(md$respond_level)
...
ggdf$condition <- md$respond_level[mm]
